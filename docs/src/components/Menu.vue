  ...
  <div :class="['v-menu', 'clearfix', classes]">
    ...
    <ul ref="menu" :class="['menu', { 'is-open': classes['is-open'] }]" :style="menuStyle" @touchstart.passive="() => {}">    
      <li
        ...
        :key="index"
        :style="route.hasNamedParent && route.level && { 'text-indent':` ${route.level}rem` }"
      >
        <router-link v-if="route.name" :to="{ name: route.name }" v-html="route.displayName"></router-link>
      </li>
  ...
  computed: {
    ...
    computedRoutes() {
      return reject(flattenRoutes(this.routes), route =>
        ...
        route.displayName = route.name ? capitalize(route.name) : undefined
        return route
      })
    },
    computedItems() {
...
a {
  ...
  transition: color 0.5s, border-color 0.5s; // Bootstrap 4 `Reboot`
